public class Texsim {
  ...
  float deactivation_increase = 0.0;
  ...
  Texsim() {
    ...
    a[Red]   = 1.0/(650.0*(1+deactivation_increase));
    a[Green] = 1.0/(1500.0*(1+deactivation_increase));
    a[Blue]  = 1.0/(600.0*(1+deactivation_increase));
    
    //deactivation parameters to deactivate magenta
    b[Red]   = 1.0/(1500.0*(1+deactivation_increase));
    b[Green] = 1.0/(600.0*(1+deactivation_increase));
    b[Blue]  = 1.0/(320.0*(1+deactivation_increase));
    
    //deactivation parameters to deactivate yellow dye
    c[Red]   = 1.0/(10000.0*(1+deactivation_increase));
    c[Green] = 1.0/(500.0*(1+deactivation_increase));
    c[Blue]  = 1.0/(30.0*(1+deactivation_increase));
    
    C_real = new Point3D(116.0,114.0,0.0);
    //C_real = new Point3D(200.0,55.0,0.0);
    C_real.scale(1.0);
